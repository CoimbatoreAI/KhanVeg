...
import { Plus, Minus, ArrowLeft, ShoppingBag, ShieldCheck, Truck, Sprout } from 'lucide-react';
import StoreLayout from '@/components/store/StoreLayout';
import { CartProvider, useCart } from '@/context/CartContext';
import ProductCard from '@/components/store/ProductCard';
import { toast } from 'sonner';

const VegProductDetailInner = () => {
    const { id } = useParams();
    ...
    if (loading) return (
        <StoreLayout store="vegetables" storeName="KHAN VEGETABLES">
            <div className="flex items-center justify-center min-h-[60vh]">
                <div className="animate-pulse text-zinc-400 uppercase tracking-widest text-xs">Authenticating Quality...</div>
            </div>
        </StoreLayout>
    );
        ...
        <StoreLayout store="vegetables" storeName="KHAN VEGETABLES">
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 xl:gap-24">
                    ...
                    >
                        <div className="absolute inset-0 bg-gradient-to-tr from-emerald-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000 z-10" />
                        <motion.img
                    ...
                    >
                        <div className="mb-8">
                            <span className="text-emerald-500 text-xs md:text-sm tracking-[0.5em] uppercase font-bold mb-4 block">
                            ...
                            <div className="h-[2px] w-20 bg-emerald-500/50 mb-8" />
                            <p className="text-zinc-500 dark:text-zinc-400 text-lg md:text-xl font-light leading-relaxed mb-8 max-w-xl">
                                {product.description}. Sourced directly from our verified organic partners to ensure the highest quality and freshness at your table.
                            </p>
                            <div className="flex items-center gap-4">
                                ...
                                <span className="text-xs text-zinc-400 uppercase tracking-widest font-medium border-l border-zinc-200 dark:border-white/10 pl-4 py-1">
                                    Incl. of all taxes<br />Free Shipping
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-6 mb-12">
                            {cartItem ? (
                                <div className="flex items-center gap-4 bg-zinc-100 dark:bg-white/5 rounded-3xl p-2 px-4 shadow-inner border border-zinc-200 dark:border-white/10">
                                    <button
                                        ...
                                        className="w-10 h-10 flex items-center justify-center text-zinc-500 hover:text-emerald-600 transition-colors"
                                    >
                                        <Minus size={20} />
                                    </button>
                                    ...
                                    <button
                                        onClick={() => updateQuantity(product._id, cartItem.quantity + 1)}
                                        className="w-10 h-10 flex items-center justify-center text-emerald-600 hover:bg-emerald-500 hover:text-white rounded-2xl transition-all"
                                    >
                                        <Plus size={20} />
                                    </button>
                                </div>
                            ...
                            >
                                <ShoppingBag size={24} className="text-zinc-400 group-hover:text-emerald-500" />
                            </Link>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-4 py-8 border-y border-zinc-100 dark:border-white/5">
                            <div className="flex flex-col items-center text-center gap-2">
                                ...
                                <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-400">100% Organic</span>
                            </div>
                            <div className="flex flex-col items-center text-center gap-2">
                                <Truck className="text-zinc-400" size={24} />
                                <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-400">Same-Day Delivery</span>
                            </div>
                            <div className="flex flex-col items-center text-center gap-2">
                                <ShieldCheck className="text-zinc-400" size={24} />
                                <span className="text-[10px] uppercase tracking-widest font-bold text-zinc-400">Quality Verified</span>
                            </div>
                        </div>
                    </motion.div>
                </div>
                ...
                <div className="mt-24 grid grid-cols-1 md:grid-cols-3 gap-12">
                    <div className="space-y-4">
                        <h3 style={{ fontFamily: '"Playfair Display", serif' }} className="text-2xl font-light">Farm Story</h3>
                        <p className="text-zinc-500 dark:text-zinc-400 font-light leading-relaxed">
                            Harvested at peak maturity from our sun-drenched valleys. We believe in minimal intervention to preserve the natural essence and nutritional profile of every {product.name.toLowerCase()}.
                        </p>
                    </div>
                    <div className="space-y-4">
                        <h3 style={{ fontFamily: '"Playfair Display", serif' }} className="text-2xl font-light">Nutritional Value</h3>
                        <ul className="text-zinc-500 dark:text-zinc-400 font-light space-y-2 text-sm">
                            <li className="flex justify-between border-b border-zinc-50 dark:border-white/5 pb-1"><span>Vitamin C</span><span>Very High</span></li>
                            <li className="flex justify-between border-b border-zinc-50 dark:border-white/5 pb-1"><span>Dietary Fiber</span><span>High</span></li>
                            <li className="flex justify-between border-b border-zinc-50 dark:border-white/5 pb-1"><span>Natural Sugars</span><span>Low</span></li>
                        </ul>
                    </div>
                    <div className="space-y-4">
                        <h3 style={{ fontFamily: '"Playfair Display", serif' }} className="text-2xl font-light">Storage Tips</h3>
                        <p className="text-zinc-500 dark:text-zinc-400 font-light leading-relaxed">
                            Store in a cool, dry place away from direct sunlight. For prolonged freshness, keep refrigerated at 4-6°C in a breathable container.
                        </p>
                    </div>
                </div>

                {/* Recommended Section (Static for now or can be added later) */}
                <div className="mt-32">
                    <div className="flex flex-col gap-2 mb-12 text-center md:text-left">
                        ...
                        <div className="h-[1px] w-20 bg-emerald-500/50 mx-auto md:mx-0" />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
                        <p className="text-zinc-400 text-xs italic py-10 col-span-full text-center">More fresh options coming soon...</p>
                    </div>
                </div>
            </div>
        </StoreLayout>
