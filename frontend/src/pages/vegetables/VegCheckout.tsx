...
const VegCheckoutInner = () => {
    ...
    useEffect(() => {
        if (!isAuthenticated) {
            ...
            navigate('/login?redirect=/vegetables/checkout');
        }
        if (items.length === 0) {
            navigate('/vegetables/cart');
        }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div className="space-y-8">
                        <section className="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-8 border border-zinc-100 dark:border-white/5 shadow-sm">
                            <h2 className="text-xs font-black uppercase tracking-[0.3em] text-emerald-600 mb-8 flex items-center gap-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-12 pr-6 py-4 bg-zinc-50 dark:bg-black/20 border border-zinc-100 dark:border-white/5 rounded-2xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all font-medium"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <textarea
                                            ...
                                            className="w-full pl-12 pr-6 py-4 bg-zinc-50 dark:bg-black/20 border border-zinc-100 dark:border-white/5 rounded-2xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition-all font-medium resize-none"
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        <section className="bg-white dark:bg-zinc-900 rounded-[2.5rem] p-8 border border-zinc-100 dark:border-white/5 shadow-sm">
                            ...
                            <div className="space-y-4">
                                <button
                                    ...
                                    className={`w-full p-4 rounded-2xl border transition-all flex items-center justify-between ${paymentMethod === 'Cash on Delivery' ? 'bg-emerald-50 dark:bg-emerald-500/5 border-emerald-100 dark:border-emerald-500/10' : 'bg-transparent border-zinc-100 dark:border-white/5 opacity-60'}`}
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-sm font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-widest">Cash on Delivery</span>
                                    </div>
                                    {paymentMethod === 'Cash on Delivery' && <CheckCircle2 className="text-emerald-500" size={20} />}
                                </button>
                                ...
                                <button
                                    ...
                                    className={`w-full p-4 rounded-2xl border transition-all flex items-center justify-between ${paymentMethod === 'Online Payment' ? 'bg-emerald-50 dark:bg-emerald-500/5 border-emerald-100 dark:border-emerald-500/10' : 'bg-transparent border-zinc-100 dark:border-white/5 opacity-60'}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-emerald-100 dark:bg-emerald-500/20 rounded-xl flex items-center justify-center text-emerald-600">
                                            💳
                                        </div>
                                        <span className="text-sm font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-widest">Online Payment</span>
                                    </div>
                                    {paymentMethod === 'Online Payment' && <CheckCircle2 className="text-emerald-500" size={20} />}
                                </button>

                                {paymentMethod === 'Online Payment' && (
                                    <div className="mt-4 p-4 bg-emerald-50/50 dark:bg-emerald-500/5 rounded-2xl border border-emerald-100 dark:border-emerald-500/10 flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                            <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Online Payment Selected</span>
                                        </div>
                                        <button onClick={() => setShowQR(true)} className="text-[10px] font-black text-emerald-500 uppercase tracking-widest hover:underline">View QR Code</button>
                                    </div>
                                )}
                            </div>
                        </section>
                    ...
                    <div className="space-y-8">
                        <section className="bg-zinc-900 text-white rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden">
                            ...
                            <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500 mb-8 border-b border-white/5 pb-4 relative z-10">Cart Summary</h2>
                            ...
                            <div className="space-y-6 relative z-10">
                                {items.map((item, idx) => (
                                    <div key={idx} className="flex justify-between items-center">
                                        ...
                                        <span className="font-mono text-emerald-500">₹{item.product.price * item.quantity}</span>
                                    </div>
                                ))}
                                ...
                                <motion.button
                                    ...
                                    disabled={loading}
                                    className="w-full bg-white text-black py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-emerald-500 hover:text-white transition-all flex items-center justify-center gap-3 disabled:opacity-50 mt-8"
                                >
                                    {loading ? <Loader2 className="animate-spin" /> : <>Finalize Order <ShoppingBag size={18} /></>}
                                </motion.button>
                            </div>
                        </section>
                        ...
                        <div className="p-6 bg-zinc-50 dark:bg-white/5 rounded-3xl border border-dashed border-zinc-200 dark:border-white/10 text-center">
                            <p className="text-[10px] text-zinc-400 uppercase font-black tracking-widest leading-relaxed">
                                Proceeding with this order creates a <br /> verified service request in our system.
                            </p>
                        </div>
                    </div>
                </div>
                        ...
                        >
                            <div className="p-8 relative">
                                ...
                                <div className="text-center space-y-6 mt-4">
                                    <div className="w-16 h-16 bg-emerald-100 dark:bg-emerald-500/20 rounded-2xl flex items-center justify-center text-emerald-600 mx-auto mb-6">
                                        <CreditCard size={32} />
                                    </div>
                                    <div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="bg-emerald-50 dark:bg-emerald-500/5 p-4 rounded-2xl border border-emerald-100 dark:border-emerald-500/10">
                                            <p className="text-[10px] text-emerald-600 font-black uppercase tracking-widest">Verification Needed</p>
                                            <p className="text-xs text-zinc-500 mt-1 italic">Please complete transaction before finalizing order</p>
                                        </div>

                                        <button
                                            onClick={() => setShowQR(false)}
                                            className="w-full py-5 bg-zinc-900 dark:bg-white text-white dark:text-black rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-xl hover:bg-emerald-500 hover:text-white transition-all"
                                        >
